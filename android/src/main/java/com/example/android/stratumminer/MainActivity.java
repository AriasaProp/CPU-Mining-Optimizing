...
public class MainActivity extends Activity {
  ...
  Thread updateThread =
      new Thread() {
        public void run() {
          ...
          // If the service is running make sure the button is changed to "Stop Mining" and vice
          // versa
        ...
        new Button.OnClickListener() {
          ...
          public void onClick(View v) {
            ...
            if (b.getText().equals(getString(R.string.status_button_start)) == true) {
              ...
              //                                                     CpuMiningWorker worker =
              // (CpuMiningWorker)mService.imw;
              //                                                     ThreadStatusAsyncTask
              // threadWaiter = new ThreadStatusAsyncTask();
              //                                                     threadWaiter.execute(worker);
            }
          }
        });
  ...
  protected void onStop() {
    ...
    if (settings.getBoolean(PREF_BACKGROUND, DEFAULT_BACKGROUND) == false) {
      if (mService != null && mService.running == true) {
        stopMining();
      }
      Intent intent = new Intent(getApplicationContext(), MinerService.class);
      stopService(intent);
  ...
  //                editor.commit();
  //
  //                startMiner(URLText, CredText, threadList.getSelectedItem().toString(), "1.0",
  // "5000", "30000");
  //            }
  //        });
  //
  ...
  //    }
  //
  //    void startMiner(String URL, String Auth, String Threads, String Throttle, String ScanTime,
  // String RetryPause)
  //    {
  ...
  //            setThreads();
  //            setUI();
  //            //startMiner("http://litecoinpool.org:9332/", "Simran.android:android", "2", "1.0",
  // "5000", "30000");
  //        }
  //    };
  //
  //    void Miner(String url, String auth, long scanTime, long retryPause, int nThread, double
  // throttle) {
  //        if (nThread < 1)
  ...
  //                float speed = (float) hashes / Math.max(1, System.currentTimeMillis() -
  // lastWorkTime);
  ...
  //            Log.i(TAG, "Usage:  java Miner [URL] [USERNAME:PASSWORD] [THREADS] [THROTTLE]
  // [SCANTIME] [RETRYPAUSE]");
  //            return;
  //        }
  //
